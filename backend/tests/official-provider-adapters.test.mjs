...
test('backend rejects unsupported providers at DB and route boundaries', () => {
  ...
  assert.match(ai, /image:\s*\[[^\]]*'\s*comfyui'\s*\]/)
  assert.match(ai, /img2img:\s*\[\s*'gemini',\s*'comfyui'\s*\]/)
  assert.match(ai, /first_last:\s*\[\s*'minimax',\s*'comfyui'\s*\]/)
  assert.match(ai, /video:\s*\[[^\]]*'\s*comfyui'\s*\]/)
  assert.match(ai, /'comfyui'/)
...
test('AI config probe uses provider-specific auth schemes', () => {
  ...
  assert.match(route, /p === 'comfyui'/)
  assert.match(route, /'\/system_stats'/)
...
test('ComfyUI adapters are registered and use API workflow files only', () => {
  const registry = read('src/services/adapters/registry.ts')
  const common = read('src/services/adapters/comfyui-common.ts')
  const imageAdapter = read('src/services/adapters/comfyui-image.ts')
  const videoAdapter = read('src/services/adapters/comfyui-video.ts')
  const route = read('src/routes/aiConfigs.ts')

  assert.match(registry, /ComfyUIImageAdapter/)
  assert.match(registry, /ComfyUIVideoAdapter/)
  assert.match(registry, /comfyui:\s*new ComfyUIImageAdapter/)
  assert.match(registry, /comfyui:\s*new ComfyUIVideoAdapter/)
  assert.match(common, /image-default\.api\.json/)
  assert.match(common, /img2img-default\.api\.json/)
  assert.match(common, /video-default\.api\.json/)
  assert.match(common, /first-last-default\.api\.json/)
  assert.match(common, /PLACEHOLDER_KEYS/)
  assert.match(common, /'PROMPT'/)
  assert.doesNotMatch(common, /from ['"].*docs\/comfyui/)
  assert.doesNotMatch(common, /readFileSync\([^)]*\.ui\.json/)
  assert.doesNotMatch(imageAdapter, /\.ui\.json/)
  assert.doesNotMatch(videoAdapter, /\.ui\.json/)
  assert.match(common, /applyBindings/)
  assert.match(common, /bindings/)
  assert.match(common, /outputBinding|bindings\?\.output/)
  assert.match(route, /comfyui-default-workflow/)
  assert.match(route, /type !== 'image' && type !== 'video' && type !== 'img2img' && type !== 'first_last'/)
  assert.match(route, /loadBuiltinWorkflowApi/)
