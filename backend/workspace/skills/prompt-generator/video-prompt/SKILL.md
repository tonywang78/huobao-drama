...
若用户消息附带 **videoEngine 引擎规范**（增量 skill），以引擎规范覆盖与本文件冲突的条款；未附带时仅遵守本底座。
若用户消息附带 **shot_style 镜头风格规范**，以风格包覆盖与本文件冲突的戏种条款；若风格包与引擎规范冲突，以引擎规范为准。
...
3-6秒：切到门口全景，门铃响，@小红推门走入，手里提着@公文包，带进一阵冷风。
6-9秒：切回中景，@小红微笑着走向小明坐下，把@公文包放到桌上，小明说：「你终于来了。」
```
...
**时间范围 ＋ 场景@引用 ＋ 景别/运镜 ＋ 角色@引用＋主体动作·表情 ＋ 道具@引用 ＋ 对白/旁白 ＋ 氛围光线**
...
- `@道具名` — 道具引用，名字必须与道具列表中的名字完全一致；**分镜已绑定且 description 中出现（被持有、使用、交接、特写或明显可见）的道具，对应 3 秒段必须写 `@道具名`，禁止只写裸名（如写「断刀」却不写 `@断刀`）**
- 生成时会自动把 `@名字` 替换为对应参考图片标记（如 `@小明` → `@图片1小明`），因此名字必须精确匹配，不要缩写或加额外符号
- **每段至少一个 @ 引用锚定画面**；角色出场的段必须 @ 该角色；绑定道具出现的段必须 @ 该道具；只引用该分镜段落已绑定的场景/角色/道具
