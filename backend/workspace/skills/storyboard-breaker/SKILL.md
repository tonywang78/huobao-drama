...
7. **画面描述** `description`：按 `【镜头1】…【镜头2】…` 逐子镜头描述观众实际看到和听到的内容——画面（谁 + 具体动作 + 肢体细节 + 表情）写在前；该子镜头有台词时以「角色名说：「台词」」写在对应 `【镜头N】` 内，旁白写「旁白：内容」。**每个 `【镜头N】` 必须单独起一行（镜头之间用换行分隔，禁止粘成一段）**
...
14. **镜头风格** `shot_style`：识别戏种后填写 `default` | `documentary` | `art_film` | `fight`；拿不准用 `default`

## 镜头风格识别（shot_style）

拆分时为每个段落判定镜头风格标签，并**按对应 `shot-styles/*` 规范写 description / atmosphere / movement**：

| 值 | 适用 |
|---|---|
| `fight` | 对打、互殴、兵器交锋、高密度攻防 |
| `documentary` | 纪实观察、流程跟拍、采访旁观感 |
| `art_film` | 情绪留白、关系张力、诗意空镜、慢节奏构图 |
| `default` | 常规叙事对话与过渡（默认） |

- 同一段落只选一个风格；风格只管镜头语法，不管项目美术外观
- 非 `default` 时必须让子镜头密度与运镜符合该风格包；`default` 遵循上文节奏分层即可
- **`shot_style=fight` 时以 fight 风格包为准**：优先长镜跟拍一镜到底，不按上文「2–4 子镜头硬切」默认拆；命中/重创才允许至多 1 次短切
...
- `description` 要适合人读，按子镜头详细描述；**每个 `【镜头N】` 单独起一行**；台词/旁白直接写在对应 `【镜头N】` 内
