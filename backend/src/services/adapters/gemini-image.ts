...
export class GeminiImageAdapter implements ImageProviderAdapter {
  ...
  buildGenerateRequest(config: AIConfig, record: ImageGenerationRecord): ProviderRequest {
    ...
    const hasReferenceImages = !!record.referenceImages && record.referenceImages !== '[]'
    ...
    // 图生图必须走多模态 generateContent（interactions 端点不支持参考图）
    if (isGemini3Image && isOfficialHost && !hasReferenceImages) {
      return {
